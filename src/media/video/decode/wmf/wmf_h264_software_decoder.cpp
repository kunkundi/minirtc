...
int WmfH264SoftwareDecoder::Decode(
  ...
  if (!configured_) {
    ...
    if (ExtractSpsPpsFromAnnexB(data, size, sps, pps)) {
      ...
      if (!avcc_.empty()) {
        if (ConfigureTypes(received_frame->Width(), received_frame->Height()) !=
            0) {
          LOG_WARN(
  ...
  DrainOutput(received_frame.get(), on_receive_decoded_frame);
  return 0;
}

int WmfH264SoftwareDecoder::CreateDecoder() {
  configured_ = false;
  started_ = false;
  decoder_.Reset();
...
int WmfH264SoftwareDecoder::ConfigureTypes(uint32_t hint_w, uint32_t hint_h) {
  if (!decoder_) {
    return -1;
  }
...
void WmfH264SoftwareDecoder::DrainOutput(
